...
def get_masks_and_scores_add_worldmodel(input_ids: torch.Tensor, tokenizer: AutoTokenizer, all_scores: List[List[float]] = None, llm_input_texts= None,observations= None,use_turn_scores: bool = False):
    """
    ...
    # Get state tokens
    start_state_tokens = [tokenizer.encode("><next_state>", add_special_tokens=False),tokenizer.encode("<next_state>", add_special_tokens=False)]
    end_state_tokens = tokenizer.encode("</next_state>", add_special_tokens=False)
    
    # Create state mask
    state_mask = torch.zeros_like(input_ids, dtype=torch.bool)
    for i in range(input_ids.shape[0]):
        # Find start positions by checking for continuous sequence of start_state_tokens
        start_positions = []
        for j in range(input_ids.shape[1] - len(start_state_tokens[0]) + 1):
            if torch.all(input_ids[i, j:j+len(start_state_tokens[0])] == torch.tensor(start_state_tokens[0], device=input_ids.device)) \
                or torch.all(input_ids[i, j:j+len(start_state_tokens[1])] == torch.tensor(start_state_tokens[1], device=input_ids.device)):
                start_positions.append(j)
        
        # Find end positions by checking for continuous sequence of end_state_tokens
        end_positions = []
        for j in range(input_ids.shape[1] - len(end_state_tokens) + 1):
            if torch.all(input_ids[i, j:j+len(end_state_tokens)] == torch.tensor(end_state_tokens, device=input_ids.device)):
                end_positions.append(j)
        # pdb.set_trace()
        # For each turn, find the first valid pair of start and end tokens
        if len(start_positions) > 0 and len(end_positions) > 0:
            # Get the turn number for each position
            start_turns = turn_indicators[i, start_positions]
            end_turns = turn_indicators[i, end_positions]
            
            # For each turn, find the first valid pair
            for turn in range(1, turn_indicators[i].max().item() + 1):
                # Find start and end positions in this turn
                turn_start_positions = [pos for pos, t in zip(start_positions, start_turns) if t == turn]
                turn_end_positions = [pos for pos, t in zip(end_positions, end_turns) if t == turn]
                if turn_start_positions and turn_end_positions:
                    # Take the first start token in this turn
                    first_start = turn_start_positions[0]
                    # Find the first end token that comes after the first start
                    valid_ends = [end for end in turn_end_positions if end > first_start]
                    if valid_ends:
                        first_end = valid_ends[0]
                        state_mask[i, first_start+len(start_state_tokens[0]):first_end] = True

    if use_turn_scores:
        for idx, scores in enumerate(list(zip(*all_scores))):
            scores = torch.tensor(scores, dtype=torch.float32, device=input_ids.device)
            ...
            scores_ = [sum(i) for i in all_scores]
            score_tensor[:, -1] = torch.tensor(scores_, dtype=torch.float32, device=input_ids.device)
    else:
        scores = [sum(i) for i in all_scores]
        score_tensor[:, -1] = torch.tensor(scores, dtype=torch.float32, device=input_ids.device)
    
    
    loss_mask = loss_mask[:, :-1] # remove the last token
    score_tensor = score_tensor[:, 1:] # remove the first token

    return loss_mask, score_tensor, response_mask, state_mask
def get_masks_and_scores_base(input_ids: torch.Tensor, tokenizer: AutoTokenizer, all_scores: List[List[float]] = None, use_turn_scores: bool = False):
    """
    input_ids: shape (bsz, seq_len)
    Get loss mask that only learns between <|im_start|>assistant and <|im_end|>. Currently only supports qwen.
    NOTE: important! This assumes that the input_ids starts with system and then user & assistant in alternative ways
    ...
    if use_turn_scores:
        for idx, scores in enumerate(list(zip(*all_scores))):
            ...
            score_tensor[reward_position] = scores
            scores_ = [sum(i) for i in all_scores]
            score_tensor[:, -1] = torch.tensor(scores_, dtype=torch.float32)
    else:
        scores = [sum(i) for i in all_scores]
...
class ContextManager:
    ...
    def __init__(self, 
        ...
        if self.config.ctx_manager.mode=='add_worldmodel':
            self.special_token_list = ["<think>", "</think>", "<observation>", "</observation>", "<prediction>", "</prediction>","<answer>", "</answer>", "<|im_start|>", "<|im_end|>"]
    ...
    def _init_prefix_lookup(self):
        ...
        for env_tag, env_config in self.config.custom_envs.items():
            ...
            for k,v in env_config.items():
                env_config_new[k] = v
            if self.config.ctx_manager.mode=='add_worldmodel':
                if env_config_new.get("env_instruction_add_worldmodel", True):
                    env_instruction = env_config_new.get("env_instruction_add_worldmodel", "")
                else:
    ...
    def _parse_response_base(self, response: str) -> List:
    ...
    def _parse_response_add_worldmodel(self, response: str) -> List:
        
        pattern = r'<think>\s*<observation>(.*?)</observation>(.*?)<prediction>(.*?)</prediction>\s*</think>\s*<answer>(.*?)</answer>' if self.config.agent_proxy.enable_think else r'<answer>(.*?)</answer>'
        match = re.search(pattern, response, re.DOTALL)
        if not match:
            answer_pattern = r'<answer>(.*?)</answer>'
            answer_match = re.search(answer_pattern, response)
            if answer_match:
                llm_response, cur_state_content, next_state_content, actions = response, [],[],answer_match.group(1)
            else:
                llm_response, cur_state_content, next_state_content, actions = response, [],[],[]
        else:
            if self.config.agent_proxy.enable_think:
                cur_state_content, think_content, next_state_content, action_content = match.group(1), match.group(2),match.group(3),match.group(4)
            else:
                cur_state_content, next_state_content, action_content = "","", match.group(1)
            for special_token in self.special_token_list:
                action_content = action_content.replace(special_token, "").strip()
                think_content = think_content.replace(special_token, "").strip()
                cur_state_content = cur_state_content.replace(special_token, "").strip()
            ...
            if len(actions) > max_actions:
                actions = actions[:max_actions] #Only the first MAX_ACTIONS actions are kept in the rollout.
                action_content = (" " + self.action_sep + " ").join(actions)
            llm_response = f"<think> <observation>{cur_state_content}</observation> {think_content} <prediction>{next_state_content}</prediction></think><answer>{action_content}</answer>" if self.config.agent_proxy.enable_think else f"<answer>{action_content}</answer>"
        return llm_response, actions, cur_state_content, next_state_content
    ...
    def _normalize_score_tensor(self, score_tensor: torch.Tensor, env_outputs: List[Dict]) -> torch.Tensor:
        ...
        # score_tensor.shape : torch.Size([128, 2627])
    ...
    def get_lm_inputs(self, env_outputs: List[Dict], prepare_for_update: bool) -> DataProto:
        ...
        for env_output in env_outputs:
            ...
            for idx, content in enumerate(env_output["history"]):
                ...
                if "state" in content:
                    ...
                    if self.config.ctx_manager.mode=='add_worldmodel':
                        FORMAT_PROMPT = "<think> <observation> [current state] </observation> [Your thoughts] <prediction> [next state]</prediction> </think> <answer> [your answer] </answer>" if self.config.agent_proxy.enable_think else "<answer> [your answer] </answer>"
                    elif self.config.ctx_manager.mode=='base':
        ...
        if prepare_for_update:
            ...
            observations = [i['state'] for i in env_output['history']]
            # observations_id = self.tokenizer(observations, return_tensors="pt", padding=T, truncation=False) # do not use padding to ensure one-to-one alignment
            if self.config.ctx_manager.mode=='add_worldmodel':
                loss_mask, score_tensor, response_mask,state_mask = get_masks_and_scores_add_worldmodel(input_ids, self.tokenizer, scores, llm_input_texts,observations,use_turn_scores=self.config.agent_proxy.use_turn_scores)
            
            elif self.config.ctx_manager.mode=='base':
                loss_mask, score_tensor, response_mask = get_masks_and_scores_base(input_ids, self.tokenizer, scores,use_turn_scores=self.config.agent_proxy.use_turn_scores)
        ...
        if prepare_for_update:
            ...
            if self.config.ctx_manager.mode=='add_worldmodel':
                llm_inputs.batch["state_mask"] = state_mask
        ...
        if self.config.ctx_manager.mode=='add_worldmodel':
            llm_inputs.non_tensor_batch["predicted_next_states"] = np.array([[None] * (len([m for m in messages if m["role"] == "assistant"])) for messages in messages_list], dtype=object)
            llm_inputs.non_tensor_batch["real_states"] = np.array([[None] * (len([m for m in messages if m["role"] == "assistant"])) for messages in messages_list], dtype=object)
            llm_inputs.non_tensor_batch["predicted_cur_states"] = np.array([[None] * (len([m for m in messages if m["role"] == "assistant"])) for messages in messages_list], dtype=object)
        # Extract next_state content from assistant messages
        for env_idx, messages in enumerate(messages_list):
            assistant_turn = 0  # Counter for assistant turns
            for message in messages:
                if message["role"] == "assistant":
                    ...
                    if self.config.ctx_manager.mode=='add_worldmodel':
                        state_match = re.search(r'<observation>(.*?)</observation>.*?<prediction>(.*?)</prediction>', content, re.DOTALL)
                        if state_match:
                            llm_inputs.non_tensor_batch["predicted_cur_states"][env_idx][assistant_turn] = state_match.group(1).strip()
                            llm_inputs.non_tensor_batch["predicted_next_states"][env_idx][assistant_turn] = state_match.group(2).strip()
                    
                    
                    assistant_turn += 1

        # Extract next_state content from User messages
        for env_idx, messages in enumerate(messages_list):
            user_turn = 0  # Counter for assistant turns
            for message in messages:
    ...
    def get_env_inputs(self, lm_outputs: DataProto) -> List[Dict]:
        ...
        if self.config.ctx_manager.mode=='add_worldmodel':
            for env_id, response in zip(env_ids, responses):
                llm_response, actions, cur_state_content, next_state_content = self._parse_response_add_worldmodel(response)
                env_inputs.append({
                    "env_id": env_id,
                    "llm_raw_response": response,
                    "llm_response": llm_response,
                    "actions": actions,
                    "llm_predict_cur_state": cur_state_content,
                    "llm_predict_next_state": next_state_content,
                })
        elif self.config.ctx_manager.mode=='base':
            for env_id, response in zip(env_ids, responses):
                llm_response, actions = self._parse_response_base(response)
                env_inputs.append({
                    "env_id": env_id,
                    "llm_raw_response": response,
                    "llm_response": llm_response,
...
@hydra.main(version_base = None, config_path = "/ssddata/shiqi/RAGEN/SPA/config", config_name = "base")
def main(config):
    import json
